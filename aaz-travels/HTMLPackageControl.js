(function () {
  ...
  function initHotelsPackage() {
    if (!window.GOL_HTMLPackageHotels) {
      return;
    }

    window.GOL_HTMLPackageHotels.init({
      encodeQueryData,
      getBasicDate,
      hideVariableDays,
      updateProperties,
  ...
  function useGlobalGOLConfigDefaultAirports() {
    ...
    const originAirportElement = originAirportsElement[0];
    originAirportElement.children[0].innerHTML = fromCities;

    const originAirportCodeElement =
      originAirportElement.children[1] || document.createElement("span");

    originAirportCodeElement.className =
      "header-search-form-inner-field-additional";
    originAirportCodeElement.innerHTML = `(${fromCodes})`;

    if (!originAirportElement.children[1]) {
      originAirportElement.appendChild(document.createTextNode(" "));
      originAirportElement.appendChild(originAirportCodeElement);
    }
  ...
  function onAddFlightButton(btn, row, rowIndex) {
    ...
    const buttonContainer =
      btn.closest(".header-search-form-multiple-add-inner-container-plus") ||
      btn.parentElement;
      ...
      if (i === lastElId || collectionLength === 1) {
        ...
        if (isLastRow) {
          ...
          lastRow
            ...
            .remove();
          const reattachedBtn = document.getElementById(
            `searchForm-multiCity-button-add-${i}`
          );
          if (reattachedBtn) {
            useGlobalGOLConfigTextStorage();
            onAddFlightButton(reattachedBtn, clonedRow || additionalRow, i + 1);
          }
        } else {
          ...
          divider.remove();
          useGlobalGOLConfigTextStorage();
          onAddFlightButton(plusButtonImg, clonedRow || additionalRow, i + 1);
        }
      }
    });
  }
    ...
    for (let week = 0; week < (days + firstDayOfMonth) / 7; week++) {
      ...
      for (
        ...
        if (week === 0 && firstDayOfMonth >= i) {
          ...
          const isToday = dayDateNormalized.getTime() === today.getTime();
          ...
          if (
            ...
            if (dayDateNormalized.getTime() < today.getTime()) {
              isDateDisabled = true;
            }
          }
          ...
          if (
            ...
            const flightStreamValue =
              flightStream !== "0" ? parseInt(flightStream, 10) : 0;
            ...
            const departureDate =
              GOL_Global.HTMLSearchForm?.flights[modifiedFlightIndex]
                ?.departure_date;
            if (departureDate) {
              const departureDateZeroTime = new Date(departureDate);
              departureDateZeroTime.setHours(0, 0, 0, 0);

              if (
                dayDateNormalized.getTime() < departureDateZeroTime.getTime()
              ) {
                isDateDisabled = true;
              }
            }
          ...
          if (isSameDay) {
            ...
            if (isToday && !isDateDisabled) {
              DayPickerDay.style.color = "#d0021b";
              DayPickerDay.style.fontWeight = "700";
            }
          } else if (isToday && !isDateDisabled) {
            DayPickerDay.style.color = "#d0021b";
            DayPickerDay.style.fontWeight = "700";
          }
        }
  ...
  function onOnewayTabClick() {
    const oneWayTab = document.getElementById("header-search-form-tab_ONE_WAY");
    ...
    oneWayTab.addEventListener("click", function () {
      ...
      document.getElementById("searchForm-multiCity").classList.add("hidden");
      document.getElementById("searchForm-standard").classList.remove("hidden");
      ...
      document.getElementById("return_date-container").classList.add("hidden");
  ...
  function onReturnTabClick() {
    const returnTab = document.getElementById("header-search-form-tab_RETURN");
  ...
  function onMultiTabClick() {
    const multiTab = document.getElementById("header-search-form-tab_MULTIPLE");
  ...
  function onDifferentReturnAirportClick() {
    const differentReturnAirport = document.getElementById(
      "header-search-form-multiCity-redirect"
    );

    if (!differentReturnAirport) {
      return;
    }

    differentReturnAirport.addEventListener("click", () => {
      GOL_Global.HTMLSearchForm.typeSearch = "MULTIPLE";
      updateProperties("#searchForm-multiCity");
      multiCityRedirect();
    });
  ...
  function logHTMLPackageDebug(message, details = {}) {
    if (!window.console) {
      return;
    }

    const logMessage = `[HTMLPackage] ${message}`;
  ...
  function logHTMLPackageError(message, details = {}) {
    if (!window.console) {
      return;
    }
    ...
    if (window.console.error) {
      window.console.error(logMessage, details);
      return;
    }
  ...
  function getGOLRequestNames(data) {
    return Object.keys(data?.GolApi?.RequestDetail || {});
  }

  function getGOLResponseSummary(responseText) {
    try {
      const response = JSON.parse(responseText);
      const responseDetail = response?.GolApi?.ResponseDetail || {};

      return {
        responseDetailNames: Object.keys(responseDetail),
  ...
  function fetchAirports({ e }) {
    ...
    const isHotelSearch = window.GOL_HTMLPackageHotels?.isActive();

    if (value.length < (isHotelSearch ? 1 : 3)) {
      return;
    }
      ...
      GolApi: {
        ...
        RequestDetail: {
          SearchDestinationsRequest_1: {
            SearchPattern: {
              $t: value,
              SearchType: isHotelSearch ? "hotel" : "flight",
            },
          },
        },
      },
  ...
  function mapSearchResultsForAutosuggestion(searchDestinationResponse) {
    const airportSuggestionsResponse =
      searchDestinationResponse.ResponseDetail.SearchDestinationsResponse_1
        ?.SearchedAirports?.SearchedAirport;
    const airportSuggestions = Array.isArray(airportSuggestionsResponse)
      ? airportSuggestionsResponse
      : airportSuggestionsResponse
      ? [airportSuggestionsResponse]
      : [];
    const codeBookAirports =
      searchDestinationResponse.CodeBook?.Airports?.Airport || [];
    const isHotelSearch = window.GOL_HTMLPackageHotels?.isActive();

    const parsedSuggestions = airportSuggestions
      .map((oSearchedAirport) => {
        const oAirportCodeBook = codeBookAirports.find(
        ...
        return {
          ...
          Showcode:
            ...
            oSearchedAirport.ShowCode === "true",
          $t: oAirportCodeBook?.$t || oSearchedAirport.Destination,
          Category: oAirportCodeBook?.Category || "CITY",
        };
      })
      .map((suggestion) => ({
        ...suggestion,
        Showcode: isHotelSearch ? false : suggestion.Showcode,
      }));

    return parsedSuggestions.map((suggestion) => {
      return {
        ...suggestion,
        Country: getCountryName(suggestion.Country),
      };
    });
  }
  ...
  function getCountryName(countryCode) {
    ...
    if (Array.isArray(countries) && countries[0]?.Code) {
      ...
      return foundCountry ? foundCountry.$t : countryCode;
    }

    if (Array.isArray(countries) && countries[0]) {
      return countries[0][countryCode] || countryCode;
    }
  ...
  function categoryIcon(category, hasMoreAirports) {
    ...
    if (window.GOL_HTMLPackageHotels?.isActive()) {
  ...
  function printFoundAirports({ foundAirports }) {
    ...
    if (foundAirports.length === 0) {
      if (!window.GOL_HTMLPackageHotels?.isActive()) {
        foundAirportsWrapper.style.display = "none";
        return;
      }

      const emptyResult = document.createElement("div");
      emptyResult.className = "header-search-form-option";
      emptyResult.innerHTML =
        GOL_Global.textStorage["HotelSelect.noCitiesFound"] ||
        "No cities found";
      foundAirportsWrapper.appendChild(emptyResult);
      return;
    }
    ...
    for (let i = 0; i < foundAirports.length; i++) {
      ...
      const country = foundAirports[Number(i)].Country;
      const displayCountry =
        country && country.length > 2 ? `, ${country}` : "";
      airportItemDiv.innerHTML = `${foundAirports[i].$t}${displayCountry}${
  ...
  async function getSelectedAirportData(e) {
    const airportOption = e.target.closest?.(".header-search-form-option");
    if (airportOption?.dataset.airportCode) {
      return airportOption.dataset;
    }
  ...
  function onSubmit() {
    ...
    if (window.GOL_HTMLPackageHotels?.isActive()) {
      window.GOL_HTMLPackageHotels.submit();
      return;
    }
    ...
    const redirectUrl = `${config.feUrl}/results?${encodeQueryData(query)}`;

    logHTMLPackageDebug("flight search redirect", {
      redirectUrl,
      typeSearch: HTMLSearchForm.typeSearch,
      lang: query.lang,
      passengerTypes: Object.keys(formatPassengersForQuery()),
      hasPreferredAirline: Boolean(HTMLSearchForm.preferred_airline),
      maxTransfers: HTMLSearchForm.max_transfers || null,
    });
  ...
  function post({ url, data, callback }) {
    const requestNames = getGOLRequestNames(data);
    const startedAt = Date.now();

    logHTMLPackageDebug("GOL API request", {
      url,
      requestNames,
      lang: data?.GolApi?.Settings?.Localization?.Language,
      country: data?.GolApi?.Settings?.Localization?.Country,
    });
    ...
    httpRequest.onreadystatechange = function () {
      ...
      if (httpRequest.readyState === XMLHttpRequest.DONE) {
        const responseDetails = {
          ...
          ...getGOLResponseSummary(httpRequest.responseText),
        };

        if (httpRequest.status === 200) {
          logHTMLPackageDebug("GOL API response", responseDetails);
          callback(httpRequest.responseText);
        } else {
          logHTMLPackageError("GOL API request failed", responseDetails);
          alert("There was a problem with the request.");
        }
      }
    };
    httpRequest.onerror = function () {
      logHTMLPackageError("GOL API network error", {
        url,
        requestNames,
        durationMs: Date.now() - startedAt,
      });
